...
class OrgMember implements RealtimeActions<OrganizationMember> {
  delete(
    param: RealtimeActionParams<OrganizationMember & { userIds: string[] }>
  ): void {
    ...
    if (
      ...
      param.data.userIds?.includes(useAuthStore.getState().user?._id)
    ) {
      useOrganizationStore.setState?.({
      ...
      useProjectStore.setState?.((prev) => ({
        ...prev,
        projectTeam: prev.projectTeam.filter(
          (team) => team.member._id !== param.data._id
        ),
        projects: prev.projects.map((app) => ({
          ...app,
