...
export default class ProjectService {
  ...
  static async getProjectRoleDefinitions(
    orgId: string
  ...
  static async deleteProject(orgId: string, projectId: string) {
    return (
      await axios.delete(`v1/org/${orgId}/project/${projectId}`, { data: {} })
    ).data;
  }

  static async leaveProjectTeam(orgId: string, projectId: string) {
    return (
      await axios.delete(`v1/org/${orgId}/project/${projectId}/team`, {
        data: {},
      })
    ).data;
  }
  static async inviteUsersToProject(
    ...
    return (
      await axios.post(
        `${this.getUrl(req.orgId, req.projectId)}/invite?uiBaseURL=${
          req.uiBaseURL
        }`,
        req.members
      )
  ...
  static async getProjectTeam(
    req: UpdateProjectParams
  ): Promise<ProjectMember[]> {
    return (await axios.get(`${this.getUrl(req.orgId, req.projectId)}/team`))
      .data;
  }

  static async changeProjectName(
    req: ChangeProjectNameRequest
  ): Promise<Project> {
    return (
      await axios.put(`${this.getUrl(req.orgId, req.projectId)}`, {
        name: req.name,
      })
    ).data;
  }
  static async setProjectAvatar(
    req: SetProjectAvatarRequest
  ): Promise<Project> {
    const formData = new FormData();
    formData.append("picture", req.picture, req.picture.name);
    return (
      await axios.put(
        `${this.getUrl(req.orgId, req.projectId)}/picture`,
        formData,
        {
          headers: {
  ...
  static async removeProjectAvatar(req: UpdateProjectParams): Promise<Project> {
    return (
      await axios.delete(`${this.getUrl(req.orgId, req.projectId)}/picture`)
    ).data;
  }
  ...
  static async removeProjectMember(req: RemoveMemberRequest) {
    return (
      await axios.delete(
        `${this.getUrl(req.orgId as string, req.projectId as string)}/team/${
          req.userId
        }`,
        {
          data: {},
  ...
  static async removeMultipleProjectMembers(
  ...
  static async updateProjectMemberRole(
    ...
    return (
      await axios.put(
        `${this.getUrl(req.orgId, req.projectId)}/team/${req.userId}`,
        {
          role: req.role,
        }
      )
    ...
    return (
      await axios.post(
        `${this.getUrl(
          req.orgId as string,
          req.projectId as string
        )}/invite/resend?token=${req.token}&uiBaseURL=${UI_BASE_URL}`,
        {
          token: req.token,
        }
      )
  ...
  static async deleteMultipleInvitations(
