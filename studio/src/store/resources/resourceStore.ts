...
type Actions = {
  ...
  testExistingResourceConnection: (
    req: AddExistingResourceRequest
  ) => Promise<void>;
  ...
  restartManagedResource: (
    req: RestartManagedResourceRequest
  ) => Promise<Resource>;
...
const useResourceStore = create<ResourceStore & Actions>()(
  devtools((set) => ({
      ...
      try {
        ...
        set((state) => ({
          resources: state.resources.filter(
            (resource) => resource._id !== req.resourceId
          ),
          isDeletedResourceModalOpen: false,
          deletedResource: null,
        }));
    ...
    updateResourceAllowedRoles: async (
      req: UpdateResourceAllowedRolesRequest
    ) => {
      try {
    ...
    updateResourceAccessSettings: async (
      req: UpdateResourceAccessSettingsRequest
    ) => {
      try {
        const resource = await ResourceService.updateResourceAccessSettings(
          req
        );
        set((state) => ({
          resources: state.resources.map((r) =>
    ...
    updateManagedResourceConfiguration: async (
      ...
      try {
        const resource = await ResourceService.updateManagedResourceConfiguration(
          req
        );
        set((state) => ({
          resources: state.resources.map((r) =>
