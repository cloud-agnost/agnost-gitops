...
export const AccessDbSchema = z
  .object({
    ...
    options: z
      ...
      .optional(),
    connFormat: z
      .nativeEnum(MongoDBConnFormat)
      .default(MongoDBConnFormat.MongoDB)
      .optional(),
    brokers: z
      .array(
        z.object({
    ...
    scheme: z
      .nativeEnum(RabbitMQConnScheme)
      .default(RabbitMQConnScheme.AMQP)
      .optional(),
  ...
  .superRefine((val, ctx) => {
    ...
    if (
      resourceConfig.instance === ResourceInstances.Kafka &&
      val.format === "ssl"
    ) {
      if (!val.ssl?.key) {
        ctx.addIssue({
...
export interface AddExistingResourceRequest {
  name?: string;
  type: ResourceType;
  instance: string;
  allowedRoles: string[];
...
export interface UpdateResourceAllowedRolesRequest {
  resourceId: string;
  allowedRoles: AllowedRole[];
  name: string;
  orgId: string;
