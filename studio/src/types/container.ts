...
export const DeploymentConfigSchema = z
	...
	.superRefine((data, ctx) => {
		if (data.cpuMetric?.enabled) {
			...
			if (['AverageValueMillicores', 'AverageValueCores'].includes(data.cpuMetric.metricType!)) {
				checkCPU(
...
export const NetworkingSchema = z
	...
	.superRefine((data, ctx) => {
		...
		if (
			[ContainerType.Deployment, ContainerType.StatefulSet].includes(type as ContainerType) &&
			!data.containerPort
		) {
			return ctx.addIssue({
				code: z.ZodIssueCode.custom,
...
export const PodConfigSchema = z
	...
	.superRefine((data, ctx) => {
		...
		checkMemory(data.memoryRequest, data.memoryRequestType, ctx, 'memoryRequest');
		...
		const memoryRequest = parseMemoryValue(data.memoryRequest, data.memoryRequestType);
		const memoryLimit = parseMemoryValue(data.memoryLimit, data.memoryLimitType);
		...
		if (memoryRequest && memoryLimit) {
			if (memoryRequest > memoryLimit) {
				ctx.addIssue({
					code: z.ZodIssueCode.custom,
					message: 'Memory request should be less than or equal to Memory limit',
					path: ['memoryRequest'],
				});
			}
		}
...
export const SourceConfigSchema = z
	.object({
		...
		watchPath: z
			.string({
				required_error: 'Watch path is required',
			})
			// Transform to process-comma-separated paths
			.transform((pathsString) => {
				// Skip transformation if undefined or empty
				if (!pathsString) return pathsString;
	...
	.superRefine((data, ctx) => {
		// all values are required if a source is connected
...
type TemplateConfig = {
	...
	defaultValues: {
		...
		'storageConfig.accessModes'?: ('ReadWriteOnce' | 'ReadOnlyMany' | 'ReadWriteMany')[];
		'statefulSetConfig.desiredReplicas'?: number;
		'statefulSetConfig.persistentVolumeClaimRetentionPolicy.whenDeleted': 'Retain' | 'Delete';
		'statefulSetConfig.persistentVolumeClaimRetentionPolicy.whenScaled': 'Retain' | 'Delete';
...
type CPUUnitType = 'millicores' | 'cores' | 'AverageValueMillicores' | 'AverageValueCores';
type MemoryUnitType = 'mebibyte' | 'gibibyte' | 'AverageValueMebibyte' | 'AverageValueGibibyte';

function checkCPU(value: number, unit: CPUUnitType, ctx: z.RefinementCtx, path: string) {
...
function checkMemory(value: number, unit: MemoryUnitType, ctx: z.RefinementCtx, path: string) {
	if (unit === 'mebibyte' || unit === 'AverageValueMebibyte') {
		...
		if (value < 10 || value > 1024) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				message: 'Memory amount must be between 10 and 1024 when using mebibytes',
				path: [path],
			});
		}
		...
		if (value < 0.1 || value > 16) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				message: 'Memory amount must be between 0.1 and 16 when using gibibytes',
				path: [path],
			});
		}
	}
...
function checkStorage(
	...
	if (unit === 'mebibyte') {
		...
		if (value < 10 || value > 1024) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				message: 'Storage request must be between 10 and 1024 when using mebibytes',
				path: [path],
			});
		}
...
export type ContainerPipelineLogStatus = 'pending' | 'running' | 'success' | 'error';
