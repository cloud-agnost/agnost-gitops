...
export const checkName = (containerType, actionType) => {
  return [
    body('name')
      ...
      .withMessage(
        `Name must be minimum ${config.get('general.minNameLength')} and maximum ${config.get(
...
export const checkRegistry = () => {
  return [
    ...
    body('registry.imageName')
      .if((value, { req }) => req.body.repoOrRegistry === 'registry' && !req.body.registry.imageUrl)
      .trim()
      .notEmpty()
      .withMessage('Required field, cannot be left empty'),
...
export const checkRepo = () => {
  return [
    body('repo.type')
      .if((value, { req }) => req.body.repoOrRegistry === 'repo' && req.body.repo.connected)
      .trim()
      .notEmpty()
      .withMessage('Required field, cannot be left empty')
      .bail()
...
export const checkVariables = () => {
  return [
    body('variables.*.name').trim().notEmpty().withMessage('Required field, cannot be left empty'),
    ...
    case 'update':
      return [
        ...
        body('networking.customDomain.domain')
          .if((value, { req }) => req.body.networking.customDomain.enabled === true)
          .notEmpty()
          .withMessage('Required field, cannot be left empty')
          .bail()
          .toLowerCase() // convert the value to lowercase
        ...
        body('networking.tcpProxy.enabled')
          .if((value, { req }) => ['deployment', 'statefulset'].includes(req.container.type))
          .trim()
          .notEmpty()
          .withMessage('Required field, cannot be left empty')
          .bail()
  ...
  if (unit === 'millicores' || unit === 'AverageValueMillicores') {
    ...
    if (
      ...
      throw new Error(
        ...
        )} and ${config.get('general.containers.maxMillicores')} when using millicores`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `CPU request must be between ${config.get('general.containers.minCPU')} and ${config.get(
          'general.containers.maxCPU'
        )} when using cores`
      );
    }
  }
...
function checkMemory(value, unit) {
  ...
  if (unit === 'mebibyte' || unit === 'AverageValueMebibyte') {
    ...
    if (
      ...
      throw new Error(
        `Memory amount must be between ${config.get('general.containers.minMiB')} and ${config.get(
          'general.containers.maxMiB'
    ...
    if (
      ...
      throw new Error(
        `Memory request must be between ${config.get('general.containers.minGiB')} and ${config.get(
          'general.containers.maxGiB'
  ...
  if (unit === 'mebibyte') {
    ...
    if (
      ...
      throw new Error(
        ...
        )} and ${config.get('general.containers.maxStorageMiB')} when using mebibytes`
      );
    }
    ...
    if (
      ...
      throw new Error(
        ...
        )} and ${config.get('general.containers.maxStorageGiB')} when using gibibytes`
      );
    }
  }
...
export const checkPodConfig = containerType => {
  return [
    body('podConfig.restartPolicy')
      ...
      .withMessage('Required field, cannot be left empty')
      .bail()
      .isIn(containerType === 'cronjob' ? ['OnFailure', 'Never'] : ['Always', 'OnFailure', 'Never'])
    ...
    body('podConfig.cpuRequest')
      ...
      .withMessage('Required field, cannot be left empty')
      .bail()
      .if((value, { req }) => ['millicores', 'cores'].includes(req.body.podConfig.cpuRequestType))
      .custom((value, { req }) => {
        checkCPU(value, req.body.podConfig.cpuRequestType);
        const request = parseCpuValue(value, req.body.podConfig?.cpuRequestType);
        const limit = parseCpuValue(req.body.podConfig.cpuLimit, req.body.podConfig.cpuLimitType);
    ...
    body('podConfig.cpuLimit')
      ...
      .withMessage('Required field, cannot be left empty')
      .bail()
      .if((value, { req }) => ['millicores', 'cores'].includes(req.body.podConfig.cpuLimitType))
      .custom((value, { req }) => {
    ...
    body('podConfig.memoryRequest')
      ...
      .custom((value, { req }) => {
        ...
        const request = parseMemoryValue(value, req.body.podConfig.memoryRequestType);
    ...
    body('podConfig.memoryLimit')
      ...
      .withMessage('Required field, cannot be left empty')
      .bail()
      .if((value, { req }) => ['mebibyte', 'gibibyte'].includes(req.body.podConfig.memoryLimitType))
      .custom((value, { req }) => {
        ...
        const limit = parseMemoryValue(value, req.body.podConfig.memoryLimitType);
...
export const checkStorageConfig = (containerType, actionType) => {
  return [
    ...
    body('storageConfig.accessModes')
      .if(
        (value, { req }) =>
          req.body.storageConfig.enabled === true && req.container.storageConfig.enabled === false
      )
      .isArray()
    ...
    body('storageConfig.sizeType')
      ...
      .bail()
      .if((value, { req }) => actionType === 'update' && req.container.storageConfig.size > 0)
      .custom((value, { req }) => {
        ...
        const existingSizeInMiB =
          existingSizeType === 'mebibyte' ? existingSize : Math.round(existingSize * 1024);
        // Calculate size in mebibytes
        const newSize = req.body.storageConfig.size;
        const newSizeType = req.body.storageConfig.sizeType;
        const newSizeInMiB = newSizeType === 'mebibyte' ? newSize : Math.round(newSize * 1024);
        ...
        if (newSizeInMiB < existingSizeInMiB && req.container.storageConfig.enabled === true) {
          throw new Error(
    ...
    body('storageConfig.size')
      ...
      .withMessage('Required field, cannot be left empty')
      .bail()
      .if((value, { req }) => ['mebibyte', 'gibibyte'].includes(req.body.storageConfig.sizeType))
      .custom((value, { req }) => {
...
export const checkDeploymentConfig = () => {
  return [
    ...
    body('deploymentConfig.cpuMetric.metricType')
      .if((value, { req }) => req.body.deploymentConfig.cpuMetric.enabled === true)
      .trim()
      .notEmpty()
      .withMessage('Required field, cannot be left empty')
      .bail()
      .isIn(['AverageUtilization', 'AverageValueMillicores', 'AverageValueCores'])
    ...
    body('deploymentConfig.cpuMetric.metricValue')
      ...
      .custom((value, { req }) => {
        ...
        } else if (req.body.deploymentConfig.cpuMetric.metricType === 'AverageUtilization') {
          if (isNaN(value)) {
    ...
    body('deploymentConfig.memoryMetric.metricType')
      .if((value, { req }) => req.body.deploymentConfig.memoryMetric.enabled === true)
      .trim()
      .notEmpty()
      .withMessage('Required field, cannot be left empty')
      .bail()
...
export const checkProbes = () => {
  return [
    ...
    body('probes.startup.initialDelaySeconds')
      ...
      .withMessage('Initial delay seconds need to be an integer between 1 and 600')
      .toInt(),
